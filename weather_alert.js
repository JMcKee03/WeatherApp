...
async function getWeatherAlerts(lat, lon, city_name) {
    try {
        ...
        if (data.features && data.features.length > 0) {
            data.features.forEach(alert => {
                document.getElementById("weather_alert_container_information").innerHTML = '';
                console.log(alert);
                const severity = alert.properties.severity; //SEVERE (RED) / MODERATE (ORANGE) ebbb02, e6731f
                let end_Time = "";

                if (alert.properties.ends) {
                    end_Time = formatDateForAlert(alert.properties.ends, alert.properties.headline); //NOW UNTIL THU 10:00 AM EST
                } else {
                    end_Time = formatDateForAlert(alert.properties.expires, alert.properties.headline);
                }

                const event = alert.properties.event.toUpperCase(); //COLD WEATHER ADVISORY or WINTER STORM WARNING

                weather_alert_container_location.innerHTML = city_name;
            ...
            weather_alert_container.classList = "hidden";
